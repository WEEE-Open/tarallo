...
GROUP BY d.Id");
		try {
			...
			for ($i = 0; $i < count($donations); $i++) {
				...
				} elseif ($donations[$i]["isCompleted"]) {
					$donations[$i]["progress"] = 100;
				} else {
					$donations[$i]["progress"] = 0;
				}
			}
			return $donations;
		} finally {
	...
	public function generateExcelSummary($id)
	{
		...
		foreach (array_unique(array_values($donation["itemsType"])) as $type) { // Good luck to anyone that will have to debug/modify this code
			...
			$itemsOfType = array_filter($donation["itemsType"], function ($it) use ($type) {
				return $it === $type;
			});
			$rootProperties = [];
			...
			if (count($countOfType) > 0) {
				$writer->writeSheetRow($displayType, array_merge(
					array_fill(0, count($rootProperties) + 1, ''),
				...
				$writer->writeSheetRow($displayType, array_merge(
					...
					array_map(function ($f) {
						return FeaturePrinter::FEATURES[$f] ?? $f;
					}, $rootProperties),
					...array_map(function ($type, $arr) use ($countOfType) {
						if (($countOfType[$type] ?? 0) > 1) {
							$acc = [];
							for ($i = 0; $i < $countOfType[$type]; $i++) {
								array_push($acc, "Id", ...array_map(function ($f) {
									return FeaturePrinter::FEATURES[$f] ?? $f;
								}, $arr));
							}
							return $acc;
						} else {
							return ["Id", ...array_map(function ($f) {
														return FeaturePrinter::FEATURES[$f] ?? $f;
							}, $arr)];
						}
				...
				foreach ($itemsOfType as $item => $_) {
					$writer->writeSheetRow($displayType, [$item,
						...array_map(function ($f) use ($item, $itemsProperties) {
							return $itemsProperties[$item]->getFeatureValue($f) ?? '';
						}, $rootProperties),
						...array_merge(...array_map(function ($type, $arr) use ($item, $countOfType, $groupedPropertiesValuesForSubItems) {
							...
							for ($i = 0; $i < $countOfType[$type]; $i++) {
								if (!isset($groupedPropertiesValuesForSubItems[$item][$type]) || count($groupedPropertiesValuesForSubItems[$item][$type]) <= $i) {
									...
									array_push($acc, $groupedPropertiesValuesForSubItems[$item][$type][$i]->getCode(), ...array_map(function ($f) use ($item, $type, $i, $groupedPropertiesValuesForSubItems) {
										return $groupedPropertiesValuesForSubItems[$item][$type][$i]->getFeatureValue($f) ?? '';
									}, $arr));
								}
							}
							return $acc;
				...
				foreach ($itemsOfType as $item => $_) {
					$writer->writeSheetRow($displayType, [$item, ...array_map(function ($f) use ($item, $itemsProperties) {
						return $itemsProperties[$item]->getFeatureValue($f) ?? '';
					}, $rootProperties)]);
				}
			}
