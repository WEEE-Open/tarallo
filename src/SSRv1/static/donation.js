(async function () {
	...
	function hydrateListInput(listInput)
	{
		...
		function renderItem(text)
		{
			...
			newItem.find("#edit").on('click', () => {
				swal({
					...
					const index = currentList.findIndex(t => (Array.isArray(t) ? t[0] : t) === (oldText || text));
					if (index > -1) {
						if (res === (Array.isArray(currentList[index]) ? currentList[index][1] : currentList[index])) {
							return;
						}
						if (res === text) {
		...
		function useLastSuggestion(e)
		{
			...
			if (newInput.dataset.autocompleteUri) {
				...
				if (e.target.value == '' || lastValid.includes(e.target.value)) {
					return;
				}
	...
	function createListInput(opt)
	{
		...
		if (opt.autocompleteUri) {
			newListInput.find('input[type=text]').get(0).dataset.autocompleteUri = opt.autocompleteUri;
		}
		if (opt.hiddenName) {
			newListInput.find('input[type=hidden]').attr("name", opt.hiddenName);
		}
		if (opt.initialData) {
			newListInput.find('input[type=hidden]').attr("value", JSON.stringify(opt.initialData));
		}
		if (opt.edit === true) {
			newListInput.find('input[type=hidden]').attr("data-edit", true);
		}
		if (opt.arrange === true) {
			newListInput.find('input[type=hidden]').attr("data-arrange", true);
		}
		...
		function refreshTypes()
		{
			...
			Object.entries(modifiedTasksCount).forEach(([type, count]) => {
				...
				if (typesCount[type] < 0) {
					typesCount[type] = 0; // this shouldn't happen but just to be safe
				}
				if (typesCount[type] === 0) {
