...
<table class="xl">
    ...
    <tr>
        <!-- owner -->
        <th>Description</th> <!-- cn -->
        <th>Token</th> <!-- uid -->
        <th>Permissions</th> <!-- uid -->
        <th>Last access</th>
        <th>Actions</th>
    </tr>
    </thead>
    <tbody>
        <?php foreach($tokens as $token): ?>
            <?php /** @var $session \WEEEOpen\Tarallo\SessionLocal */ $session = $token['Session']; ?>
            <?php /** @var $lastAccess DateTime|null */ $lastAccess = $token['LastAccess']; ?>
            <td><?= $this->e($session->description) ?></td>
            <td><?= $this->e($token['Token']) ?>:****************</td>
